...
function main(mp)
    ...
    for realization in 1:mp.realizations
        ...
        for Tstep in 1:mp.Tsteps
            beta=1/(mp.Tmax-mp.deltaT*(Tstep-1))
            ...
            mean_C_t=mean(C_t)
            C[realization,Tstep]=beta*beta*mean_C_t
            std_C[realization,Tstep]=beta*beta*std(C_t,corrected=true,mean=mean_C_t)
    ...
    #println(yerr)
    writedlm("SHN$(mp.N)T$(mp.Tmin)_$(mp.Tmax).dat", [range(mp.Tmin,stop=mp.Tmax,length=mp.Tsteps) y' yerr'])
end
using DelimitedFiles
using Statistics
const  l=2
const N=18
const realizations=1
const samples=100
const repeats=250
const steps=300
const Tmin=1
...
modpara=mp(l,N,realizations,samples,repeats,steps,Tmin,Tmax,Tsteps,deltaT)
@time main(modpara)
